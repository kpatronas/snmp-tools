...
def main():
    ...
    try:
        ...
        if (session.ErrorStr):
            ...
            sys.exit(2)
        print("RAM  (Total / Free): "+convertSize(int(memory_res[2]))+" / "+convertSize((int(memory_res[4])+int(memory_res[6])+int(memory_res[7]))-int(memory_res[0]))+"\nSwap (Total / Free): "+convertSize(int(memory_res[0]))+" / "+convertSize(int(memory_res[1])))
